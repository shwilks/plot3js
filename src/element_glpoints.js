...
import { DefaultArray, DefaultColor, DefaultProperties } from './plotting.js';
import { Sprites } from './sprites.js';
import { Shaders } from './shaders.js';

// GL line constructor
export const constructors_glpoints = function(
    plotobj,
    viewer
    ){
    ...
    if (plotobj.shape != "sphere") {
        var glpoints = new element_glpoints({
...
export const element_glpoints = class GLPoints {
    ...
    constructor(args){
        ...
        // Set default args
        args.shape = DefaultArray(args.shape, "circle", coords.length);
        if(!args.order){ args.order = new Array(coords.length).fill(0).map((x,i) => i) }

        // Set default properties
        args.properties              = DefaultProperties(args.properties, coords.length);
        args.properties.bgcolor      = DefaultColor(args.properties.bgcolor, coords.length);
        args.properties.visible      = DefaultArray(args.properties.visible, 1, coords.length);
        args.properties.aspect       = DefaultArray(args.properties.aspect, 1, coords.length);
        args.properties.outlinewidth = DefaultArray(args.properties.outlinewidth, 1, coords.length);
        ...
        if (args.dimensions == 2) {
            ...
            var material = new THREE.ShaderMaterial( { 
                uniforms: { 
                    ...
                    viewportHeight: { value: vheight },
                    viewportPixelRatio: { value: pixelratio },
                    circleTexture: { value: Sprites("ball", viewport.viewer) }
                }, 
                vertexShader:   renderer.shaders.vertexShader,
                fragmentShader: Shaders.FragmentShader2D,
                alphaTest: 0.9,
                transparent: true,
                blending: THREE.NormalBlending
            } );
...
// Make a thin line object
export const element_glpoint = class GLPoint extends element_base {
